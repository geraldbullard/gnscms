<?php
 /**
  * Language: French
  */

  /* Commun */  
  $lang['admin'] = 'Admin';
  $lang['afternoon_cms'] = 'Afternoon CMS';
  $lang['appearance'] = 'Apparence';
  $lang['charisma'] = 'Charisma';
  $lang['content'] = 'Contenu';
  $lang['copyright'] = 'Droit d\'Auteur';
  $lang['copyright_title'] = 'le Meilleur dans le Developpement Web';
  $lang['core_code'] = 'Code de Base Sur la Base de';
  $lang['core_code_title'] = 'Code du Noyau Base sur "Creer un CMS a un apres-midi avec PHP et MySQL" par Matt Doyle'; 
  $lang['dashboard'] = 'Tableau de Bord';
  $lang['edit'] = 'Edit';
  $lang['file_manager'] = 'Gestionnaire de Fichiers';
  $lang['gallery'] = 'Galerie';
  $lang['gnscms'] = 'gnsCMS';
  $lang['group'] = 'Groupe';
  $lang['groups'] = 'Groupes';
  $lang['hide_navigation'] = 'Masquer la Navigation';
  $lang['language'] = 'French';
  $lang['list'] = 'Liste';
  $lang['logout'] = 'Deconnexion';
  $lang['manage_content'] = utf8_encode('Gérer le Contenu');
  $lang['manage_files'] = utf8_encode('Gérer les Fichiers');
  $lang['manage_galleries'] = utf8_encode('Gérer Galeries');
  $lang['manage_groups'] = utf8_encode('Gérer les Groupes');
  $lang['manage_settings'] = utf8_encode('Gérer les Parametres');
  $lang['manage_themes'] = utf8_encode('Gérer les Themes');
  $lang['manage_users'] = utf8_encode('Gérer les Utilisateurs');
  $lang['navigation'] = 'Navigation';
  $lang['powered_by'] = 'Administrateur Alimente par';
  $lang['powered_by_title'] = 'Open Source, la Peau Mutiple, Modele d\'Administration de Repondre Pleinement';
  $lang['search'] = 'Recherche';
  $lang['settings'] = 'Reglages';
  $lang['search_content'] = 'Recherche de Contenu';
  $lang['show_navigation'] = 'Afficher la Navigation';
  $lang['themes'] = 'Themes';
  $lang['tools'] = 'Outils';
  $lang['user'] = 'Utilisateur';
  $lang['users'] = 'Utilisateurs';
  $lang['view_dashboard'] = 'Vior Tableau de Bord';
  $lang['view_site'] = 'Voir le Site';
  $lang['view_site_new_window'] = 'Voir le Site dans une Nouvelle Fenetre';
  
  /* Connectez-vous Page */
  $lang['login'] = 'Connexion';
  $lang['login_title'] = 'Connexion Administration';
  $lang['login_meta_description'] = 'Administration par le maestro';
  $lang['login_remove_install'] = utf8_encode('S\'il vous plaît supprimer install.php immédiatement!');
  $lang['login_wrong_username'] = utf8_encode('Mauvais nom d\'utilisateur, mot de passe ou condition.<br />S\'il vous plaît contactez l\'administrateur du site.');
  $lang['login_logged_out'] = utf8_encode('Vous êtes connecté Out.');
  $lang['login_not_logged_in'] = utf8_encode('Vous n\'êtes pas connecté.');
  $lang['login_session_ended'] = utf8_encode('Votre session est terminée. Connectez-vous nouveau.');
  $lang['login_thanks_for_trying'] = 'Merci d\'essayer gnsCMS!';
  $lang['login_no_direct_access'] = utf8_encode('Pas d\'accès direct à ce fichier!');
  $lang['login_enter_user_pass'] = 'Entrez votre nom d\'utilisateur et mot de passe pour vous connecter.';
  
  echo $gns_admin_RCI->get('frlang', 'add');  
?>